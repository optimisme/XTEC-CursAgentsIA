...
NODE
  ...
  if [ "$HARNESS_STATUS" -eq 0 ] && [ "$IS_EDIT_PROMPT" -eq 1 ]; then
...
NODE
  ...
  if [ "$HARNESS_STATUS" -eq 0 ] && [ "$IS_EXISTING_BEHAVIOR_MODIFY" -eq 1 ]; then
...
NODE
  ...
  if [ "$HARNESS_STATUS" -eq 0 ] && [ -s "$CHANGED_WEB_FILES" ]; then
    if ! node .opencode/harness/web_post_edit_check.js "$CHANGED_WEB_FILES"; then
