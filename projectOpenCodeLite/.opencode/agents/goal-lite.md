...
3. Use subagents only for focused web research or read-only web quality review.
4. Use safe-edit for every file change.
...
13. In `lines`, put one physical file line per item. Do not place `\n` inside a `lines` item.
14. For JS longer than about 80 lines, create a small scaffold first, then add functions in chunks of about 25-50 short lines.
15. No placeholders, npm installs, invented validators, or long plans.
