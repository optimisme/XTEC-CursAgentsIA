...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| `compose-qwen38-27b-cuda-vram128-vllm-qwen-fp8-64k-image.yml` | Qwen3.8 27B Qwen FP8 | vLLM | 128 GB | 64k | 8 | 2096 | 0.75 | Si | 10 | FP8 oficial Qwen, KV cache al dtype del model perquè el checkpoint no inclou escales FP8 KV calibrades, prefill fragmentat a 2096 per estabilitat GDN en Blackwell, mostreig thinking oficial (`temperature=1.0`, `top_p=0.95`, `top_k=20`, `min_p=0`, penalitzacions 0/1), MTP nadiu d'una capa amb un token especulatiu, `qwen3` + `qwen3_coder` i sense DFlash. |
...
| `compose-qwen36-27b-aeon-ultimate-uncensored-multimodal-cuda-vram128-vllm-aeon-nvfp4-mtp-xs-64k-image.yml` | Qwen3.6 27B AEON NVFP4 MTP-XS | vLLM | 128 GB | 64k | 32 | 65536 | 0.80 | Si | 7 | Variant AEON multimodal amb la mateixa configuracio 64k + image del perfil Qwen3.6 35B NVFP4; validada a Spark, pero molt mes lenta a OpenCode: mitjana 157.44 s per crida, 10.88 tok/s wall, i parser `qwen3_xml` amb mes soroll. |
...
| `compose-qwen36-35b-a3b-base-cuda-vram16-llamacpp-localweights-iq4-mtp-image.yml` | Qwen3.6 35B A3B GGUF IQ4_XS | llama.cpp | 16 GB | 64k | 1 | n/d | n/d | Si | 10 | Mateixa capacitat alta de programació i més context; menys concurrència i throughput per `-np 1` i offload parcial (`-ngl 24`). Imatge habilitada amb `mmproj-BF16.gguf` d'Unsloth i `--no-mmproj-offload` per reduir VRAM. |
| `compose-ornith10-9b-cuda-vram16-llamacpp-deepreinforce-gguf-q8-image.yml` | Ornith-1.0 9B Q8_0 GGUF | llama.cpp | 16 GB | 64k | 1 | n/d | n/d | Si | 8 | Variant image amb quant Q8_0 d'uns 9.53 GB i mmproj Qwen3.5 9B en CPU; perfil de prova per veure si encaixa en 16GB. |
| `compose-ornith10-9b-cuda-vram16-llamacpp-deepreinforce-gguf-q6k-image.yml` | Ornith-1.0 9B Q6_K GGUF | llama.cpp | 16 GB | 64k | 1 | n/d | n/d | Si | 7 | Model dense DeepReinforce orientat a coding agents; quant Q6_K d'uns 7.36 GB, imatge amb mmproj Qwen3.5 9B en CPU. |
| `compose-ornith10-9b-cuda-vram16-llamacpp-deepreinforce-gguf-q6k.yml` | Ornith-1.0 9B Q6_K GGUF | llama.cpp | 16 GB | 64k | 1 | n/d | n/d | No | 7 | Variant text-only del perfil Q6_K, sense mmproj. |
